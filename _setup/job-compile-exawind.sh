#!/bin/bash
#SBATCH --job-name=CEX
#SBATCH --time=0-04:00:00  # Job time limit Days-Hours:Minutes:Seconds
...
if [[ "$CREATE_ENV" == "true" ]]; then
    ...
    spack manager create-env --name $ENV_NAME -cpu --spec 'exawind@master~amr_wind_gpu~cuda~gpu-aware-mpi~nalu_wind_gpu ^amr-wind@main~cuda~gpu-aware-mpi+hypre+mpi+netcdf+shared ^nalu-wind@master~cuda~fftw~gpu-aware-mpi+hypre+shared ^tioga@develop %oneapi' || exit 1
    # --- Check dependencies 

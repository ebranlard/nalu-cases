...
airfoil_names = ['S809','du00-w-212', 'nlf1-0416', 'ffa-w3-211']


fig, axes = plt.subplots( 4, 3, gridspec_kw={'width_ratios': [1, 5, 1]}, figsize=(12.8, 3.9))
fig.subplots_adjust(left=0.04, right=0.99, top=0.999, bottom=0.01, hspace=0.04, wspace=0.20)
...
# 
    ...
    arf.tri_plot(     title=None, axes=axes[i,:], n_target=20, legend=False)
# 
...
for i,ax in enumerate(np.array(axes)[:,0]):
    ax.set_aspect('equal', 'box')
for i,ax in enumerate(np.array(axes)[:,2]):
    ax.set_aspect('equal', 'box')

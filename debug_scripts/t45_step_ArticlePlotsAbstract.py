...
# def step_response_H34_1(t, V, c, pWag, Clalpha=2*np.pi):
#     """
#     Step response assuming H3/4 = 1.
#     This corresponds to the standard Wagner/Jones approximation.
#     """
#     A1, b1, A2, b2 = pWag
#     beta1 = b1 * (2 * V / c)
#     beta2 = b2 * (2 * V / c)
#     
#     # Standard Wagner function response: 1 - A1*exp(-beta1*t) - A2*exp(-beta2*t)
#     # This assumes A1 + A2 = 0.5 for the Wagner limit at t=0
#     cl_step = Clalpha * (1 - A1 * np.exp(-beta1 * t) - A2 * np.exp(-beta2 * t))
#     return cl_step
# 
# def step_response_H34_Tu(t, V, c, pWag, Tu, Clalpha=2*np.pi):
#     """
#     Step response using H3/4 = 1 + Tu*s.
#     This accounts for the phase/lead term in the transfer function.
#     """
#     A1, b1, A2, b2 = pWag
#     beta1 = b1 * (2 * V / c)
#     beta2 = b2 * (2 * V / c)
#     
#     # Integration of the impulse response h4(t) derived:
#     # The term (Tu * sum(Ai*betai)) delta(t) becomes a step jump at t=0.
#     # The exponential terms integrate to (Ai*betai*(1-Tu*betai)/betai) * (1 - exp(-betai*t))
#     
#     # Constant term from the delta function integration
#     jump = Tu * (A1 * beta1 + A2 * beta2)
#     
#     # Integrated exponential terms
#     term1 = A1 * (1 - Tu * beta1) * (1 - np.exp(-beta1 * t))
#     term2 = A2 * (1 - Tu * beta2) * (1 - np.exp(-beta2 * t))
#     
#     cl_step = Clalpha * (jump + term1 + term2)
#     return cl_step

def step_response_H34_1(s, pWag, Clalpha=1):
    """
    Step response assuming H3/4 = 1.
    """
    A1, b1, A2, b2 = pWag
    # cl_step / Clalpha = 1 - sum(Ai * exp(-bi * tau))
    cl_step = Clalpha * (1 - A1 * np.exp(-b1 * s) - A2 * np.exp(-b2 * s))
    return cl_step

def step_response_H34_Tu(s, pWag, Clalpha=1):
    """
    Step response using H3/4 = 1 + Tu*s.
    Derived from the impulse response h4(t) integrated over time.
    """
    A1, b1, A2, b2 = pWag
    
    # Jump term at t=0 derived from the delta function: sum(Ai * bi)
    jump = A1 * b1 + A2 * b2
    
    # Integrated exponential terms: Ai * (1 - bi) * (1 - exp(-bi * tau))
    term1 = A1 * (1 - b1) * np.exp(-b1 * s)
    term2 = A2 * (1 - b2) * np.exp(-b2 * s)
    
    cl_step = Clalpha * (1 - term1 - term2)
    return cl_step

# s_th=np.linspace(0, 20,100)

# PhiOF = step_response_H34_1(s_th, pOF, 1)
PhiCorOF = step_response_H34_Tu(s_th, pOF, 1)
PhiCorWag = step_response_H34_Tu(s_th, pWag, 1)
PhiCorKus = step_response_H34_Tu(s_th, pWag, 1)

# 
# fig, ax = plt.subplots(1, 1, sharey=False, figsize=(6.4,4.8))
# fig.subplots_adjust(left=0.12, right=0.95, top=0.95, bottom=0.11, hspace=0.20, wspace=0.20)
# ax.plot(s_th, H1    , label='1')
# ax.plot(s_th, H2    , label='34')
# ax.plot(s_th, PhiWagner,'--'    , label='Wag')
# ax.set_xlabel('')
# ax.set_ylabel('')
# ax.set_xlim([0,20])
# ax.legend()
...
fig=None
datCFD=None
datUA={0:None, 2:None, 3:None, 4:None, 5:None}
UAMods = [0, 2, 3, 5]
...
if 0 in IPlot:
    ...
    plotDatAgg(ax, datUA[0], label='UA0', color=COLUA0   , ls='-.')
...
if 1 in IPlot:
    ...
    fig.subplots_adjust(left=0.1, right=0.97, top=0.99, bottom=0.125, hspace=0.20, wspace=0.20)
    plotDatAgg(ax, datCFD, label='CFD', color=fColrs(1))
    plotDatAgg(ax, datUA[5], label='UA5', color=fColrs(4), lw=3, ls='-')
    ax.plot(s_th, PhiCorOF  , '--', label='Theory', lw=2, c='k')
#     ax.plot(s_th, PhiWagner , '-',  label='Wagner ' , lw=2, c=(0.5,0.5,0.5))
#     ax.plot(s_th, PhiKussner, ':',  label='Kussner' , lw=2, c=(0.5,0.5,0.5))
#     ax.plot(s_th, PhiOF     , '--', label='Eq. 4 - OpenFAST', lw=2, c=(0.5,0.5,0.5))
    ax.legend()
    ax.set_ylabel(r"Normalized lift $\phi(s)$ [-]")
    ax.set_xlabel(r"Dimensionless time, $s=2 U t / c$ [-]")
    ax.set_xlim([-0.0, 20])

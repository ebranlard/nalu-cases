...
NOTE: this script uses CFD_OUTB so you need to run t41 first to create the .outb... Not clean..
...
setFigurePath('../article2_nawea_cfd_airfoil/figs/')
setFigureFont(12)
setFigureTitle(1)
HR=False
HR=True
...
def plotDatAgg(axes, dat, label=None, color='k', lw=1.5, ls='-', alpha=0.3, dwells=True, bg=True, alphaReg=None):
    if dat is None:
...
#         axes[1].plot(        k, dat['phi'][i], c=color, label=label, lw=lw, ls=ls)
    ...
    axes[0].plot(        k, Hrlmean, c=color, label=label, lw=lw, ls=ls, alpha=alphaReg)
    axes[1].plot(        k, phimean, c=color, label=label, lw=lw, ls=ls, alpha=alphaReg)
...
if HR:
    ...
    cases+=[{'airfoil_name':'S809'       , 'n':24 , 're':0.75, 'suffix':'_HR', 'Cl_alpha':6.50842, 'alpha0':-1.00410  }] # NOTE: 0.8 or 0.75
...
# fig1, axes =  plt.subplots(2, 1, sharey=False, figsize=(6.4,4.8))
...
for cs in cases:
    ...
    if cs['re']==0.75:
        yml_path  = '_results/cases_chirp_n{}/{}/{}_re{:05.2f}_mean00_A01{:s}.yaml'.format(cs['n'], cs['airfoil_name'], cs['airfoil_name'], cs['re'], cs['suffix'])
    else:
...
plotDatAgg(axes , datUA[0]     , label='UA0'   , color=COLUA0      , ls=None  , lw = 3.5, dwells=False , bg=False, alphaReg=1.0)
plotDatAgg(axes , datUA[3]     , label = 'UA3' , color = fColrs(3) , ls = '-' , lw = 3.5, dwells=False , bg=False, alphaReg=0.8)
plotDatAgg(axes , datUA['5OF'] , label = 'UA5' , color = fColrs(4) , ls = '-' , lw = 3.5, dwells=False , bg=False, alphaReg=0.5)
...
if isinstance(filtCutOff, str):
    filtCutOff=0.5

print('>>> C_nalpha', pol['C_nalpha'])
print('>>> filtCut ', filtCutOff)
print('>>> k_ta    ', pol['re'])
pTh = pOF

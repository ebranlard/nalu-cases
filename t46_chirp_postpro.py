...
def plotme(ax, x, y, **kwargs):
    if log:
        if scale0:
            ax.plot(x, 20*np.log10(y/y[0]), **kwargs)
        else:
...
def postpro_chirp_tf(ch, dw, info, st=None, plot=False, label='CFD', fig=None, ls='-', c=fColrs(1), lw=1.5, marker=None):
    ...
    if plot:
        ...
        if scale0:
            ax1.plot(k_ch[mask], 20*np.log10(mag_ch[mask]/mag_ch[0]), ls=ls, lw=lw, c=c, label = label, marker=marker)
        else:
            ax1.plot(k_ch[mask], 20*np.log10(mag_ch[mask]), ls=ls, lw=lw, c=c, label = label, marker=marker)
        ...
        if dw is not None:
            for ii, dwi in enumerate(dw_h):
                n = len(dwi['mag'])-1
                if scale0:
                    ax1.plot([dwi['k']]*n, 20*np.log10(dwi['mag'][1:]/mag_ch[0]), '.', c=c) 
                else:
                    ax1.plot([dwi['k']]*n, 20*np.log10(dwi['mag'][1:]), '.', c=c) 
...
#     pTh = pKus
    ...
    if scale0:
        fig.axes[0].plot(out['k'], 20*np.log10(out_th['mag']/out_th['mag'][1]), label='Theory Wag', c='k')
    else:
        fig.axes[0].plot(out['k'], 20*np.log10(out_th['mag']), label='Theory Wag', c='k')
    ...
    out_th = get_analytical_tf(f_th, U=info['U'], Cl_alpha=Cl_alpha, A1=pTh[0], A2=pTh[2], b1=pTh[1], b2=pTh[3], chord=chord)
    if scale0:
        fig.axes[0].plot(out['k'], 20*np.log10(out_th['mag']/out_th['mag'][1]), label='Theory OF', c='k', ls='--')
    else:
        fig.axes[0].plot(out['k'], 20*np.log10(out_th['mag']), label='Theory OF', c='k', ls='--')
    fig.axes[1].plot(out['k'],             out_th['phi']                  , label='Theory OF', c='k', ls='--')

    pTh = pKus
    out_th = get_analytical_tf(f_th, U=info['U'], Cl_alpha=Cl_alpha, A1=pTh[0], A2=pTh[2], b1=pTh[1], b2=pTh[3], chord=chord)
    if scale0:
        fig.axes[0].plot(out['k'], 20*np.log10(out_th['mag']/out_th['mag'][1]), label='Theory Kus', c='k', ls=':')
    else:
        fig.axes[0].plot(out['k'], 20*np.log10(out_th['mag'])                 , label='Theory Kus', c='k', ls=':')
